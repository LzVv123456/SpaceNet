...
def DataGenerator(image_path, mask_path, augmentation_image, augmentation_mask, input_type, img_size=(256, 256),
    ...
    if prepare_array:
        for img in glob.glob(image_path + '/' + input_type + '/*.jpg'):
            ...
            mask = cv2.imread(mask_path + '/map/' + name + '.png', 0)
            # mask = np.load(mask_path + '/distance/' + name + '.npy')
        ...
        train_img, val_img, train_mask, val_mask = train_test_split(all_images, all_masks, test_size=0.3)
