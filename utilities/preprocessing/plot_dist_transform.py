...
###############################################################################
def plot_dist_transform(input_image, pixel_coords, dist_image,mask_image,
                        figsize=(8, 8), plot_name='', add_title=False,
                        colorbar=True,
                        poly_face_color='orange', poly_edge_color='red',
                        poly_nofill_color='blue', cmap='jet'):
    ...
    # transform
    # cbar_pointer = ax1.imshow(dist_image,cmap=cmap)
    ...
    # ax.imshow(dist_image,cmap=cmap)
    # truth polygons
    if len(patches) > 0:
    ...
    if len(plot_name) > 0:
        plt.imsave(plot_name,img,cmap='gray')
